...
export const ideaCandidateOutputSchema = z.object({ id: idSchema, workingTitle: z.string().min(1).max(300), angle: z.string().min(1).max(2000), corePromise: z.string().min(1).max(1000), viewerProblem: z.string().min(1).max(1000), dramaticQuestion: z.string().min(1).max(1000), targetEmotion: z.string().min(1).max(300), trafficModel: z.enum(["browse", "suggested", "search", "mixed"]), thumbnailConcept: z.string().min(1).max(1000), noveltyExplanation: z.string().min(1).max(1000), noveltyScore: z.number().min(0).max(100), audienceFitScore: z.number().min(0).max(100), thumbnailPotentialScore: z.number().min(0).max(100), productionFeasibilityScore: z.number().min(0).max(100), scoreExplanations: z.object({ novelty: z.string().min(1).max(1000), audienceFit: z.string().min(1).max(1000), thumbnailPotential: z.string().min(1).max(1000), productionFeasibility: z.string().min(1).max(1000) }).strict(), researchRisk: z.enum(["low", "medium", "high"]), productionDifficulty: z.enum(["low", "medium", "high"]), repurposePotential: z.array(z.string().min(1).max(300)).max(20), whyThisCanWin: z.string().min(1).max(1000) }).strict();
...
export const storyBeatSchema = z.object({ id: idSchema, order: z.number().int().nonnegative(), purpose: z.string().min(1).max(1000), information: z.string().min(1).max(3000), tensionRole: z.string().min(1).max(1000), revealRole: z.string().min(1).max(1000), viewerQuestion: z.string().min(1).max(1000) }).strict();
export const storyArchitectureOutputSchema = z.object({ premise: z.string().min(1).max(2000), viewerPromise: z.string().min(1).max(2000), dramaticQuestion: z.string().min(1).max(2000), audienceStartingState: z.string().min(1).max(2000), audienceEndingState: z.string().min(1).max(2000), narrativeStrategy: z.string().min(1).max(4000), beats: z.array(storyBeatSchema).min(2).max(30), openLoops: z.array(z.string().min(1).max(1000)).max(20), payoff: z.string().min(1).max(2000), emotionalArc: z.array(z.string().min(1).max(1000)).min(2).max(30) }).strict().superRefine((value, context) => { const orders = value.beats.map((beat) => beat.order); if (new Set(value.beats.map((beat) => beat.id)).size !== value.beats.length) context.addIssue({ code: z.ZodIssueCode.custom, message: "Story beat IDs must be unique." }); if (orders.some((order, index) => order !== index)) context.addIssue({ code: z.ZodIssueCode.custom, message: "Story beats must use consecutive order values." }); });
export const storyArchitectureRequestSchema = z.object({ projectId: idSchema }).strict();
export const storyArchitectureArtifactResponseSchema = z.object({ id: idSchema, stageRunId: idSchema.optional(), status: workflowArtifactStatusSchema, payloadJson: storyArchitectureOutputSchema, createdAt: z.string(), updatedAt: z.string() }).strict();
export const storyArchitectureArtifactsResponseSchema = z.array(storyArchitectureArtifactResponseSchema);
export const outlineSectionSchema = z.object({ id: idSchema, order: z.number().int().nonnegative(), purpose: z.string().min(1).max(1000), narrativeRole: z.string().min(1).max(1000), keyPoint: z.string().min(1).max(3000), storyBeatIds: z.array(idSchema).min(1).max(30), linkedClaimIds: z.array(idSchema).max(100), dramaticFunction: z.string().min(1).max(1000), estimatedWords: z.number().int().positive().max(10000), estimatedSeconds: z.number().int().positive().max(7200), retentionIntent: z.string().min(1).max(1000), openLoop: z.string().max(1000).optional(), proofObjects: z.array(z.string().min(1).max(1000)).max(50) }).strict();
export const outlineOutputSchema = z.object({ targetDurationSeconds: z.number().int().positive().max(7200), targetWords: z.number().int().positive().max(100000), assumedWordsPerMinute: z.number().int().min(80).max(240), sections: z.array(outlineSectionSchema).min(1).max(100) }).strict();
...
export const scriptOutputSchema = z.object({
  sections: z.array(scriptSectionOutputSchema).min(1).max(100),
  source: z.enum(["provider", "user_edit", "revision", "imported"]).optional(),
  parentScriptArtifactId: idSchema.optional(),
  selectedFindingIds: z.array(idSchema).max(100).optional()
}).strict();
...
export const scriptReviewFindingSchema = z.object({ id: idSchema, severity: z.enum(["low", "medium", "high", "blocking"]), sectionId: idSchema.optional(), category: z.enum(["hook", "promise", "pacing", "density", "repetition", "open_loop", "payoff", "clarity", "claim", "originality", "tone", "retention"]), explanation: z.string().min(1).max(3000), suggestedDirection: z.string().min(1).max(3000) }).strict();
export const scriptReviewOutputSchema = z.object({ overallStatus: z.enum(["pass", "needs_changes", "blocked"]), summary: z.string().min(1).max(4000), findings: z.array(scriptReviewFindingSchema).max(100), strengths: z.array(z.string().min(1).max(2000)).max(50), requiredFixes: z.array(z.string().min(1).max(2000)).max(50), optionalImprovements: z.array(z.string().min(1).max(2000)).max(50) }).strict();
export const scriptReviewArtifactPayloadSchema = scriptReviewOutputSchema.extend({ scriptArtifactId: idSchema }).strict();
export const scriptReviewRequestSchema = z.object({ projectId: idSchema, scriptArtifactId: idSchema.optional() }).strict();
export const scriptReviewArtifactResponseSchema = z.object({ id: idSchema, stageRunId: idSchema.optional(), status: workflowArtifactStatusSchema, payloadJson: scriptReviewArtifactPayloadSchema, createdAt: z.string(), updatedAt: z.string() }).strict();
export const scriptReviewArtifactsResponseSchema = z.array(scriptReviewArtifactResponseSchema);
export const reviseScriptRequestSchema = z.object({ projectId: idSchema, scriptArtifactId: idSchema, reviewArtifactId: idSchema, findingIds: z.array(idSchema).min(1).max(100), instructions: z.string().trim().max(4000).optional() }).strict();
export const approveScriptRequestSchema = z.object({ projectId: idSchema, scriptArtifactId: idSchema }).strict();
