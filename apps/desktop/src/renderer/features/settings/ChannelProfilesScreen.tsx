...
import { channelStylePresets, characterVersionIsApproved, createDefaultChannelDna, getChannelStylePreset, motionEffectsFromGrammar, normalizeChannelDna, recommendChannelDirection } from "@lsf/domain";
        ...
        {selected ? (
          <SectionCard title="Thiết lập nhân vật kênh" description="Tạo prompt cục bộ, tạo ảnh trong GG Lab, tải từng góc nhìn rồi duyệt cả bộ.">
            ...
            <SectionCard title="Channel DNA" description="Một nguồn sự thật cho nội dung, style, nhân vật, asset và mặc định dựng video.">
              <div className="form-grid">
                <FormField label="Visual style" htmlFor="channel-style"><select id="channel-style" value={channelDna.visualStyle.styleId} onChange={(event) => { const styleId = event.target.value as ChannelDna["visualStyle"]["styleId"]; const preset = getChannelStylePreset(styleId); const preferredMotion = motionEffectsFromGrammar(preset.motionGrammar)[0] ?? "zoom_in"; setChannelDna((current) => ({ ...current, visualStyle: { ...current.visualStyle, styleId, name: preset.name, description: preset.description, sceneGrammar: [...preset.sceneGrammar], motionGrammar: [...preset.motionGrammar], palette: [...preset.defaultPalette], productionProfile: preset.productionProfile }, contentDirection: { ...current.contentDirection, pillars: [...preset.defaultPillars], defaultAngles: [...preset.contentGrammar] }, productionDefaults: { ...current.productionDefaults, defaultMotion: preferredMotion, frameDensity: { min: preset.productionProfile.frameDensity.min, max: preset.productionProfile.frameDensity.max }, transitionStyle: preset.motionGrammar[0] ?? "cut" } })); }} >{channelStylePresets.map((preset) => <option key={preset.id} value={preset.id}>{preset.name}</option>)}</select></FormField>
