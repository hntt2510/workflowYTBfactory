...
export interface OutlineArtifact { id: string; stageRunId?: string; status: "draft" | "needs_review" | "needs_attention" | "approved" | "rejected" | "stale"; payloadJson: { targetDurationSeconds: number; targetWords: number; assumedWordsPerMinute: number; sections: Array<{ id: string; purpose: string; narrativeRole: string; keyPoint: string; storyBeatIds: string[]; linkedClaimIds: string[]; estimatedWords: number; estimatedSeconds: number; retentionIntent: string }> }; createdAt: string; updatedAt: string; }
export interface ScriptArtifact { id: string; stageRunId?: string; status: "draft" | "needs_review" | "needs_attention" | "approved" | "rejected" | "stale"; payloadJson: { source?: "provider" | "user_edit" | "revision" | "imported"; parentScriptArtifactId?: string; selectedFindingIds?: string[]; sections: Array<{ id: string; outlineSectionId: string; purpose: string; narration: string; estimatedWords: number; estimatedSeconds: number; linkedClaimIds: string[]; dramaticFunction: string; visualOpportunities: string[]; proofObjects: string[]; retentionRisk: "low" | "medium" | "high" }> }; createdAt: string; updatedAt: string; }
export interface StoryArchitectureArtifact { id: string; stageRunId?: string; status: "draft" | "needs_review" | "needs_attention" | "approved" | "rejected" | "stale"; payloadJson: { premise: string; viewerPromise: string; dramaticQuestion: string; narrativeStrategy: string; beats: Array<{ id: string; order: number; purpose: string; information: string; tensionRole: string; revealRole: string; viewerQuestion: string }>; payoff: string; emotionalArc: string[] }; createdAt: string; updatedAt: string; }
...
export interface ScriptReviewArtifact { id: string; stageRunId?: string; status: "draft" | "needs_review" | "needs_attention" | "approved" | "rejected" | "stale"; payloadJson: { scriptArtifactId: string; overallStatus: "pass" | "needs_changes" | "blocked"; summary: string; findings: Array<{ id: string; severity: "low" | "medium" | "high" | "blocking"; sectionId?: string; category: string; explanation: string; suggestedDirection: string }>; strengths: string[]; requiredFixes: string[]; optionalImprovements: string[] }; createdAt: string; updatedAt: string; }
...
export interface LongShortFactoryApi {
  ...
  runStoryArchitecture: (input: { projectId: string }) => Promise<FactoryProject>;
  listStoryArchitectureArtifacts: (input: { projectId: string }) => Promise<StoryArchitectureArtifact[]>;
  approveStoryArchitecture: (input: { projectId: string }) => Promise<FactoryProject>;
  rejectStoryArchitecture: (input: { projectId: string }) => Promise<FactoryProject>;
  ...
  approveScript: (input: { projectId: string; scriptArtifactId: string }) => Promise<FactoryProject>;
  rejectScript: (input: { projectId: string }) => Promise<FactoryProject>;
  runScriptReview: (input: { projectId: string; scriptArtifactId?: string }) => Promise<FactoryProject>;
  listScriptReviewArtifacts: (input: { projectId: string }) => Promise<ScriptReviewArtifact[]>;
  reviseScript: (input: { projectId: string; scriptArtifactId: string; reviewArtifactId: string; findingIds: string[]; instructions?: string }) => Promise<FactoryProject>;
