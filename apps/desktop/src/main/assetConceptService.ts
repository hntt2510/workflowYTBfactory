...
function buildAssetConceptRequest(shots: AssetConceptShot[], character: CharacterVersion): string {
  return `Create a concise visual asset concept for every shot. Return strict JSON {"concepts":[...]}. Each concept must use exactly {"id":string,"shotId":string,"semanticBeat":string,"kind":"object"|"diagram"|"background"|"teacher_gesture"|"text_card","role":string,"description":string,"visualConstraints":string[],"colorPalette":string[],"motionIntent":string,"needsReferenceImage":boolean}. Use the same character identity across teaching gestures. Do not create final image prompts. Character lock: ${JSON.stringify({ name: character.name, persona: character.persona, invariantTraits: character.invariantTraits, prohibitedChanges: character.prohibitedChanges })}. Shots: ${JSON.stringify(shots)}`;
}
...
export async function runAssetConcepts(input: {
  ...
  if (input.manualMode) {
    return { output: assetConceptsOutputSchema.parse({ concepts: input.shots.map((shot) => deterministicAssetConcept(shot)) }) };
  }
  ...
  try {
    responses = await Promise.all(batches.map((batch) => client.createResponseText({ model: settings.textModel!, timeoutMs: assetConceptTimeoutMs, input: buildAssetConceptRequest(batch, input.character) })));
  } catch (error) {
...
function deterministicAssetConcept(shot: AssetConceptShot) {
  ...
  return {
    ...
    motionIntent: "Use a restrained pan or zoom to support the narration; do not invent a new action.",
