import type { ChannelDna, CharacterVersion } from "@lsf/domain";
...
function buildAssetConceptRequest(shots: AssetConceptShot[], character: CharacterVersion, channelDna?: ChannelDna): string {
  return `Create a concise visual asset concept for every shot. Use the Channel DNA asset grammar, palette, and motion grammar. Return strict JSON {"concepts":[...]}. Each concept must use exactly {"id":string,"shotId":string,"semanticBeat":string,"kind":"object"|"diagram"|"background"|"teacher_gesture"|"text_card","role":string,"description":string,"visualConstraints":string[],"colorPalette":string[],"motionIntent":string,"needsReferenceImage":boolean}. Use the same character identity across teaching gestures. Do not create final image prompts. Character lock: ${JSON.stringify({ name: character.name, persona: character.persona, invariantTraits: character.invariantTraits, prohibitedChanges: character.prohibitedChanges })}. Channel DNA: ${JSON.stringify(channelDna)}. Shots: ${JSON.stringify(shots)}`;
}
...
export async function runAssetConcepts(input: {
  ...
  if (input.manualMode) {
    return { output: assetConceptsOutputSchema.parse({ concepts: input.shots.map((shot) => deterministicAssetConcept(shot, input.channelDna)) }) };
  }
  ...
  try {
    responses = await Promise.all(batches.map((batch) => client.createResponseText({ model: settings.textModel!, timeoutMs: assetConceptTimeoutMs, input: buildAssetConceptRequest(batch, input.character, input.channelDna) })));
  } catch (error) {
...
function deterministicAssetConcept(shot: AssetConceptShot, channelDna?: ChannelDna) {
  ...
  return {
    ...
    colorPalette: ["warm coral accent", "charcoal", "cream"],
    motionIntent: `Use ${channelDna?.productionDefaults.defaultMotion ?? "zoom_in"} with the channel motion grammar; do not invent a new action.`,
