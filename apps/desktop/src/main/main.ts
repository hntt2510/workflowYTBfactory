...
ipcMain.handle("run-outline", async (_event, input: unknown) => {
  ...
  const fingerprint = canonicalSha256({ stageId: "outline", ideaArtifactId: ideaArtifact.id, profileId: profile.id, promptProfileVersion: profile.channelPromptProfile?.version, duration: project.setup.targetDuration, language: projectLanguage }); const existing = workflowRunStore.findLatestByInput(projectId, "outline", fingerprint); if (isPendingOrAcceptedRun(existing)) return factoryProjectResponseSchema.parse(project);
...
ipcMain.handle("run-shot-plan", async (_event, input: unknown) => {
  ...
  const fingerprint = canonicalSha256({ stageId: "shot-plan", sceneArtifactId: sceneArtifact.id, promptProfileVersion: project.setup.channelPromptProfileSnapshot?.version, fps: project.timeline.fps, visualWorkflow: project.setup.visualWorkflow });
...
ipcMain.handle("run-asset-concepts", async (_event, input: unknown) => {
  ...
  const fingerprint = canonicalSha256({ stageId: "asset-concepts", visualArtifactId: visualArtifact.id, characterVersionId: character.id, promptProfileVersion: project.setup.channelPromptProfileSnapshot?.version });
