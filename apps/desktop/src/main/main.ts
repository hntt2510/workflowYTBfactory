...
function resolveProjectPromptContext(project: FactoryProject, profile: ChannelProfile | undefined, taskType: "story" | "director" | "scene_prompt" | "scene_image_generation" | "build", scene?: { characterIds?: string[]; assetIds?: string[]; continuityLocks?: string[]; forbiddenChanges?: string[] }): ResolvedChannelPromptContext | undefined {
  const channelId = project.setup.channelId;
  const promptProfile = project.setup.channelPromptProfileSnapshot
    ?? (channelId && project.setup.channelDnaSnapshot && profile
      ? buildChannelPromptProfile({ channelId, channelDna: project.setup.channelDnaSnapshot, niche: profile.niche, ...(profile.characterVersions ? { characterVersions: profile.characterVersions } : {}), ...(profile.activeCharacterVersionId ? { activeCharacterVersionId: profile.activeCharacterVersionId } : {}), version: profile.channelPromptProfile?.version ?? 1 })
      : profile?.channelPromptProfile);
  if (!channelId || !promptProfile) return undefined;
  if (promptProfile.channelId !== channelId) throw new Error("Project Prompt Profile does not belong to the selected channel.");
  return resolveChannelPromptContext({ channelId, profile: promptProfile, taskType, projectSnapshotVersion: promptProfile.version, ...(scene ? { scene } : {}) });
}
...
ipcMain.handle("run-shot-plan", async (_event, input: unknown) => {
  ...
  const fingerprint = canonicalSha256({ stageId: "shot-plan", sceneArtifactId: sceneArtifact.id, fps: project.timeline.fps, visualWorkflow: project.setup.visualWorkflow });
  ...
  try {
    const result = await runShotPlan({ scenes: scenePlan.scenes, fps: project.timeline.fps, characterFirst: project.setup.visualWorkflow === "character_first", credentialStore, certificationStore: textCertificationStore });
    const artifactId = "artifact-" + randomUUID();
    const finishedAt = new Date().toISOString();
...
ipcMain.handle("run-asset-concepts", async (_event, input: unknown) => {
  ...
  const fingerprint = canonicalSha256({ stageId: "asset-concepts", visualArtifactId: visualArtifact.id, characterVersionId: character.id });
