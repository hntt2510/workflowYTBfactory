...
import { outlineOutputSchema, resolveChannelPromptContext, type ChannelDna, type ChannelPromptProfile, type ResolvedChannelPromptContext } from "@lsf/domain";
...
export async function runOutline(input: { idea: Record<string, unknown>; claims: Array<{ id: string; state: string; approvalState: string }>; profile: Record<string, unknown>; channelDna?: ChannelDna; promptContext?: ResolvedChannelPromptContext; targetDuration: string; language: string; credentialStore: ProviderCredentialStore; certificationStore: TextCertificationStore; createClient?: (config: { baseUrl: string; apiKey: string }) => TextClient }) {
  const channelDna = input.channelDna ?? (input.profile as { channelDna?: ChannelDna }).channelDna;
  const profilePrompt = (input.profile as { id?: string; niche?: string; channelPromptProfile?: ChannelPromptProfile }).channelPromptProfile;
  const promptContext = input.promptContext ?? (profilePrompt && (input.profile as { id?: string }).id
    ? resolveChannelPromptContext({ channelId: (input.profile as { id: string }).id, profile: profilePrompt, taskType: "story" })
    : undefined);
  const storyContext = promptContext ? {
    channelId: promptContext.channelId,
    contentIdentity: promptContext.contentIdentity,
    contentLane: promptContext.contentLane,
    storyPattern: promptContext.storyPattern
  } : undefined;
  ...
  let response: { text: string; returnedModelId?: string }; try { response = await (input.createClient?.({ baseUrl: settings.baseUrl, apiKey }) ?? new NineRouterClient({ baseUrl: settings.baseUrl, apiKey, timeoutMs: 60_000 })).createResponseText({ model: settings.textModel, input: `Create an outline only from the approved idea and channel story context. The target language is ${input.language}. This is a hard requirement: write every human-readable field in ${input.language}; do not write the outline in English unless English is the target language. Respect only the resolved channel content identity, content lane, and story pattern when choosing section purposes and proof objects. Return strict JSON {"sections":[...]}. Every section must use exactly {"id":string,"order":integer starting at 0,"purpose":string,"keyPoint":string,"linkedClaimIds":string[],"dramaticFunction":string,"estimatedSeconds":positive integer,"openLoop"?:string,"proofObjects":string[]}. linkedClaimIds may be empty because this workflow does not use web research or Claim Map. Do not fact-check, browse, create claims, scenes, shots, or image prompts. Respect the requested duration.\n${JSON.stringify({ idea: input.idea, channelStoryContext: storyContext, targetDuration: input.targetDuration, targetLanguage: input.language })}` }); } catch (error) { throw new OutlineError("provider_failed", error instanceof NineRouterTextResponseError ? `Outline provider request failed: ${error.status}.` : "Outline provider request failed."); }
