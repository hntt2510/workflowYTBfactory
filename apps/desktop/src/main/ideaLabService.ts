...
export async function runIdeaLab(input: { opportunityMap: Record<string, unknown>; topic: string; profile: Record<string, unknown>; format: string; language: string; targetDuration: string; credentialStore: ProviderCredentialStore; certificationStore: TextCertificationStore; createProvider?: () => TextProvider }) {
  ...
  try {
    response = await (input.createProvider?.() ?? configured.provider).generateText({ model: configured.model, input: `Generate exactly 6 original YouTube idea candidates from this approved Opportunity Map and active channel context: exactly 2 researchRisk low, 2 medium, and 2 high. Do not copy competitor wording or thumbnails, promise guarantees, defame, or auto-select an idea. Return only strict JSON {"candidates":[...]}. Each candidate must use exactly these fields: id, workingTitle, angle, corePromise, viewerProblem, dramaticQuestion, targetEmotion, trafficModel (browse|suggested|search|mixed), thumbnailConcept, noveltyExplanation, noveltyScore (0-100), audienceFitScore (0-100), thumbnailPotentialScore (0-100), researchRisk (low|medium|high), productionDifficulty (low|medium|high), repurposePotential (string[]), whyThisCanWin. Use unique stable IDs and do not replace these keys with title, concept, hook, or format.\n${JSON.stringify({ opportunityMap: input.opportunityMap, topic: input.topic, profile: input.profile, format: input.format, language: input.language, targetDuration: input.targetDuration })}` });
  } catch { throw new IdeaLabError("provider_failed", "Idea Lab text provider request failed."); }
  let parsed: unknown; try { parsed = JSON.parse(response.text.trim()); } catch { throw new IdeaLabError("invalid_json", "Idea Lab returned invalid JSON."); }
  const result = ideaLabOutputSchema.safeParse(parsed); if (!result.success) throw new IdeaLabError("invalid_output", "Idea Lab returned an invalid candidate set.");
