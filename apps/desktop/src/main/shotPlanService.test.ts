...
describe("shot plan service", () => {
  ...
  it("routes character-first motion through the selected channel grammar", async () => { const { db, credentialStore, certificationStore } = await setup(); const profile = buildChannelPromptProfile({ channelId: "milo-red-panda", channelDna: createDefaultChannelDna({ styleId: "cute-daily-life-cartoon" }) }); const promptContext = resolveChannelPromptContext({ channelId: profile.channelId, profile, taskType: "director" }); const output = { shots: [{ id: "shot-1", sceneId: "scene-1", order: 0, startFrame: 0, durationFrames: 60, fps: 30, purpose: "Cash flow rises", visualMode: "ai_image", framing: "Medium", cameraAngle: "Eye level", cameraMovement: "Static", subjectAction: "Increase", startState: {}, endState: {}, continuityRefs: [] }] }; const result = await runShotPlan({ scenes: [{ id: "scene-1", startFrame: 0, durationFrames: 60 }], fps: 30, characterFirst: true, promptContext, credentialStore, certificationStore, createClient: () => ({ createResponseText: async () => ({ text: JSON.stringify(output) }) }) }); expect(result.output.shots[0]?.motion?.effect).toBe("pop"); db.close(); });
